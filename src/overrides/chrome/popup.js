...
browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === 'customStatus' || message.action === 'streamPopupStatus') {
    ...
    if (isLargeTarget) {
      if (!hlsLarge) {
        if (typeof Hls !== 'undefined' && Hls.isSupported()) {
          ...
          await new Promise(resolve => hlsLarge.on(Hls.Events.MANIFEST_PARSED, resolve));
      ...
      html += `
        <mdui-card href="${link.url}" target="_blank" clickable style="background: var(--surface-low); border: 1px solid rgb(var(--mdui-color-outline-variant)); border-radius: 12px; transition: var(--transition); height: 100%;">
          <div style="padding: 12px; display: flex; align-items: center; gap: 12px; height: 100%; box-sizing: border-box;">
            ...
            <span style="font-size: 0.85rem; font-weight: 600; line-height: 1.2; color: rgb(var(--mdui-color-on-surface));">${localizedLabel}</span>
          </div>
        </mdui-card>
      `;
    });
...
async function downloadAllAsZip(items) {
  ...
  try {
    ...
    // Keep the work in this UI when background downloading is disabled.
        ...
        if (activeTab && activeTab.url && activeTab.url.startsWith('http')) {
            ...
            if (response && response.pong) {
                const backgroundJob = await browser.runtime.sendMessage({
                    ...
                    isYouTube: isYouTube,
                    audioOnly: true,
                    directAudioSource: false,
                    encodeM4aToMp3: false,
  ...
  try {
   ...
   const defaultName = targetRequest.pageTitle || getFileName(url, 100);
    ...
    try {
        ...
        if (isYouTube) {
            const backgroundJob = await browser.runtime.sendMessage({
                action: 'startPersistentAudioJob',
                url,
                audioUrl: audioUrl && audioUrl !== 'all' ? audioUrl : null,
                filename: newName,
                isYouTube: true,
                audioOnly: false,
                request: targetRequest,
    ...
    if (error.message && error.message.includes("Extension context invalidated")) {
        errMsg = "Extension context invalidated. Please reload the page to continue using the downloader.";
    }
    try {
        showDialog(browser.i18n.getMessage("downloadError", [errMsg]) || errMsg);
    } catch (e) {
        showDialog(errMsg);
    }
    if (downloadId) finishDownloadUI(downloadId);
  } finally {
    if (window.activeCancellations) {
        window.activeCancellations.delete(url);
...
async function getM3u8BlobUrlFromMpd(mpdUrl, headers) {
  try {
    ...
    function getPlaylistForAdaptationSet(asNode) {
      const reps = Array.from(asNode.getElementsByTagNameNS(NS, "Representation").length ? asNode.getElementsByTagNameNS(NS, "Representation") : asNode.getElementsByTagName("Representation"));
    ...
    const videoPlay = videoSet ? getPlaylistForAdaptationSet(videoSet) : null;
    const audioPlay = audioSet ? getPlaylistForAdaptationSet(audioSet) : null;
    
    if (!videoPlay && !audioPlay) throw new Error("Could not parse any media track from MPD");
    
    if (videoPlay && audioPlay) {
      const videoBlob = new Blob([videoPlay.m3u8], { type: "application/x-mpegURL" });
      ...
      master += `#EXT-X-STREAM-INF:BANDWIDTH=${videoPlay.bandwidth || 1000000},AUDIO="audio"\n${videoUrl}\n`;
      
      return URL.createObjectURL(new Blob([master], { type: "application/x-mpegURL" }));
    } else {
      const singlePlay = videoPlay || audioPlay;
      const blob = new Blob([singlePlay.m3u8], { type: "application/x-mpegURL" });
