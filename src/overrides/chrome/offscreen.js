...
browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.action === 'triggerOffscreenDownload') {
        ...
        handleOffscreenDownload(id, filename)
            .then((result) => sendResponse({ success: true, url: result.url, filename: result.filename }))
...
async function handleOffscreenDownload(id, filename) {
    try {
        ...
        // Revoke the object URL after 60 seconds to prevent memory leaks.
        // The service worker will download the blob during this window.
        setTimeout(() => {
