...
},{"./ssa.js":7}],2:[function(require,module,exports){
'use strict';
...
subsrt.format['vtt'] = require('./format/vtt.js');
subsrt.format['lrc'] = require('./format/lrc.js');
subsrt.format['smi'] = require('./format/smi.js');
subsrt.format['ssa'] = require('./format/ssa.js');
subsrt.format['ass'] = require('./format/ass.js');
subsrt.format['sub'] = require('./format/sub.js');
subsrt.format['srt'] = require('./format/srt.js');
subsrt.format['sbv'] = require('./format/sbv.js');
subsrt.format['json'] = require('./format/json.js');

subsrt.list = function() { return Object.keys(subsrt.format); };
subsrt.detect = function(content) {
  var formats = subsrt.list();
  for (var i = 0; i < formats.length; i++) {
    var f = formats[i];
    var handler = subsrt.format[f];
    if (typeof handler == "undefined" || typeof handler.detect != "function") continue;
...
subsrt.parse = function(content, options) {
  ...
  if (!format || format.trim().length == 0) throw new Error("Cannot determine subtitle format!");
  var handler = subsrt.format[format];
  if (typeof handler == "undefined") throw new Error("Unsupported subtitle format: " + format);
  var func = handler.parse;
  if (typeof func != "function") throw new Error("Subtitle format does not support 'parse' op: " + format);
...
subsrt.build = function(captions, options) {
  ...
  if (typeof func != "function") throw new Error("Subtitle format does not support 'build' op: " + format);
...
subsrt.convert = function(content, options) {
  ...
  delete opt.format;
  if (opt.from) opt.format = opt.from;
  var captions = subsrt.parse(content, opt);
  if (opt.resync) captions = subsrt.resync(captions, opt.resync);
  opt.format = opt.to || options.format;
  return subsrt.build(captions, opt);
};
subsrt.resync = function(captions, options) {
  options = options || { };
  var func, ratio, frame, offset;
  if (typeof options == "function") func = options;
  else if (typeof options == "number") {
    offset = options;
    func = function(a) { return [ a[0] + offset, a[1] + offset ]; };
  } else if (typeof options == "object") {
    offset = (options.offset || 0) * (options.frame ? options.fps || 25 : 1);
    ratio = options.ratio || 1.0;
    frame = options.frame;
    func = function(a) { return [ Math.round(a[0] * ratio + offset), Math.round(a[1] * ratio + offset) ]; };
    ...
    if (typeof caption.type === "undefined" || caption.type == "caption") {
      if (frame) {
        ...
        if (shift && shift.length == 2) {
          caption.frame.start = shift[0]; caption.frame.end = shift[1];
          caption.frame.count = caption.frame.end - caption.frame.start;
        }
      } else {
        var shift = func([ caption.start, caption.end ]);
        if (shift && shift.length == 2) {
          caption.start = shift[0]; caption.end = shift[1];
          caption.duration = caption.end - caption.start;
        }
      }
    }
...
},{"./format/ass.js":1,"./format/json.js":2,"./format/lrc.js":3,"./format/sbv.js":4,"./format/smi.js":5,"./format/srt.js":6,"./format/ssa.js":7,"./format/sub.js":8,"./format/vtt.js":9}],11:[function(require,module,exports){
var subsrt = require('subsrt');
subsrt.format.vtt = require('subsrt/lib/format/vtt');
subsrt.format.lrc = require('subsrt/lib/format/lrc');
subsrt.format.smi = require('subsrt/lib/format/smi');
subsrt.format.ssa = require('subsrt/lib/format/ssa');
subsrt.format.ass = require('subsrt/lib/format/ass');
subsrt.format.sub = require('subsrt/lib/format/sub');
subsrt.format.srt = require('subsrt/lib/format/srt');
subsrt.format.sbv = require('subsrt/lib/format/sbv');
subsrt.format.json = require('subsrt/lib/format/json');
window.subsrt = subsrt;

},{"subsrt":10,"subsrt/lib/format/ass":1,"subsrt/lib/format/json":2,"subsrt/lib/format/lrc":3,"subsrt/lib/format/sbv":4,"subsrt/lib/format/smi":5,"subsrt/lib/format/srt":6,"subsrt/lib/format/ssa":7,"subsrt/lib/format/sub":8,"subsrt/lib/format/vtt":9}]},{},[11]);
